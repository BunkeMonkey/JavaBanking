...
public class KontoOberflaeche extends BorderPane {
	...
	private OberflaechenController controller;

	private Konto model;

	private BooleanProperty negativeBalance;
	
	/**
	 * erstellt die Oberfläche
	 */
	public KontoOberflaeche(OberflaechenController controller, Konto model)
	{
		...
		nummer = new Text(String.valueOf(model.getKontonummer()));
		...
		meldung.setFill(Color.FORESTGREEN);
		...
		//TODO: should abheben and einzahlen be bidirectional?
		einzahlen.disableProperty().bindBidirectional(model.gesperrtProperty());
		abheben.disableProperty().bindBidirectional(model.gesperrtProperty());
		...
		stand.fillProperty().bind(model.negativeBalanceProperty().map(negBalance -> negBalance ? Color.RED : Color.GREEN));
		stand.underlineProperty().bind(model.negativeBalanceProperty());


		einzahlen.setOnAction(e -> controller.einzahlen(betrag.getText()));
		abheben.setOnAction(e -> controller.abheben(betrag.getText()));
	...
	/**
	 * setzt die Meldung
	 * @param noticeText der Text der Meldung
	 * @param color die Farbe der Meldung
	 * @param underline ob die Meldung unterstrichen werden soll
	 */
	public void setMeldung(String noticeText, Color color, boolean underline){
		meldung.setText(noticeText);
		meldung.setFill(color);
		meldung.setUnderline(underline);
	}
}
